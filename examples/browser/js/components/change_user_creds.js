...
export const ChangeUserCredentialsPanel = () => {
	...
	const [oldPrivateKeyReadOnly, setOldPrivateKeyReadOnly] = useState(true);
	...
	const handleSendBtn = (e) => {
		...
		let oldPrivateKeyBytes;
		try {
			oldPrivateKeyBytes = b64ToArrayBuffer(oldPrivateKey);
			if (oldPrivateKeyBytes.byteLength !== 32) {
				throw new Error();
			}
			setOldPrivateKeyError('');
		} catch {
			setOldPrivateKeyError('Missing or invalid old private key.');
			hasError = true;
		}
		...
		client
			.changeUserCredentialsCommand(oldPrivateKeyBytes, newPublicKeyBytes)
			.then(() => {
				if (isMounted) {
					showToast('User credentials successfully changed.', 'success');
					setOldPrivateKeyError('');
					setNewPublicKey('');
			...
			.catch((err) => {
				if (isMounted) {
					...
					console.error('Change user credentials |', err);
					setWaitingReply(false);
				}
			});
	};
	...
	return html`
		<article>
			<header>Change user credentials</header>
				...
				<fieldset>
					<label>
						...
						<input
							type="password"
							value=${oldPrivateKey}
							onInput=${handleOldPrivateKeyInput}
							placeholder="Base64-encoded old private key"
							disabled=${waitingReply}
							autocomplete="off"
							aria-invalid=${oldPrivateKeyError ? 'true' : null}
							readonly=${oldPrivateKeyReadOnly}
							onfocus=${() => setOldPrivateKeyReadOnly(false)}
						/>
						${
							oldPrivateKeyError &&
							html`
							<small role="status" className='pico-color-red-500'>${oldPrivateKeyError}</small>
						`
						}
					</label>
					<label>
